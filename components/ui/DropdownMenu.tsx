...
const DropdownMenu = ({ options, selected, onSelect }: DropdownMenuProps) => {
  ...
  return (
    <div class="relative w-full" ref={ref}>
      ...
      >
        {(() => {
          const current = options.find(opt => typeof window !== "undefined" && window.location.pathname === opt.value);
      ...
      {open && (
        <ul
          class="absolute z-10 left-0 right-0 mt-2 bg-white border border-base-200 rounded shadow-lg"
        >
          {options.map((opt, idx) => (
            <li key={idx}>
              <a
                href={opt.value}
                class={`block px-4 py-3 text-sm ${typeof window !== "undefined" && window.location.pathname === opt.value ? "bg-primary/10 text-primary font-bold" : "hover:bg-gray-100"}`}
                onClick={(e) => {
                  e.preventDefault();
                  setOpen(false);
